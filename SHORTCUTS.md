...
- 2026-05-31 view/launchpad: only looperPad takes the full PadEvent (to read Velocity); drumPad/sessionPad/settingsPad/projectPad/emptyPad still take (row, col, down). Migrate the rest to the full PadEvent when they need velocity/aftertouch so the handler signatures are uniform again.
- 2026-05-31 DESIGN/looper.md specifies "a note held at the loop seam gets a clean note-off at the boundary" (no hung notes, no wrap). NOT implemented — the looper just records the NoteOn/NoteOff the player plays; a note held across the loop end will currently behave however playback happens to read it. Needs the playback/compile engine to emit a boundary note-off. Revisit when the playback engine handles held notes.
